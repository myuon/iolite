...
declare fun extcall_ui_main(ui: rawptr);
declare fun extcall_window_new(ui: rawptr, title_ptr: rawptr, title_len: int, width: int, height: int): rawptr;
...
module UI {
  fun init(): UI {
    return UI(extcall_ui_init());
  }

  fun main(self) {
    return extcall_ui_main(self.!);
  }
}
...
module Window {
  fun build(ui: UI, title: array[byte], width: int, height: int): Window {
    return Window(extcall_window_new(ui.!, title.ptr as rawptr, title.length, width, height));
  }

  fun set_child(self, child: rawptr) {
    return extcall_window_set_child(self.!, child);
  }
